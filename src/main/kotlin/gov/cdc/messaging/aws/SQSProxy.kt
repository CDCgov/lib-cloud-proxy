...
class SQSProxy(private val awsConfig: AWSConfig, private val meterRegistry: MeterRegistry? = null) : CloudMessaging {
    ...
    override fun receiveMessage(queueName: String): List<CloudMessage> =
        meterRegistry.withMetricsTimer("sqs.receiveMessage.poll") { timer ->
            kotlin.runCatching {
                when {
                    queueName.startsWith("https://sqs") -> queueName
                    else -> getQueueUrl(queueName)
                }.let { url ->
                    logger.trace("Receiving messages on queue: {}", queueName)
                ...
                logger.error("Failed to receive message on queueUrl: {}. Exception: {}", queueName, it)
            }.getOrThrow()
        }
